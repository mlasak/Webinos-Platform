...
public class AppLauncherManagerImpl extends AppLauncherManager implements IModule {
	...
	public void launchApplication(
		...
		try{
			...
			Log.v("AppLauncherManagerImpl", "1");
			i.setData(Uri.parse(app));
			i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
			Log.v("AppLauncherManagerImpl", "2");
			androidContext.startActivity(i);
			Log.v("AppLauncherManagerImpl", "3");
			successCallback.handleEvent("");
			Log.v("AppLauncherManagerImpl", "4");
		}
		catch (Exception e){
			Log.v("AppLauncherManagerImpl", "5");
			errorCallback.handleEvent("");
			Log.v("AppLauncherManagerImpl", "6");
		}
